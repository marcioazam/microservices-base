...
func PersonNameLens() Lens[Person, string] {
	return NewLens(
...
func PersonAgeLens() Lens[Person, int] {
	return NewLens(
...
func PersonAddressLens() Lens[Person, Address] {
	return NewLens(
...
func TestLensComposition(t *testing.T) {
	t.Run("Compose creates nested lens", func(t *testing.T) {
		...
		personCity := Compose(personAddr, addrCity)
