...
func (sw *SlidingWindow) Allow(ctx context.Context, key string) (resilience.RateLimitDecision, error) {
	sw.mu.Lock()
	defer sw.mu.Unlock()
...
func (sw *SlidingWindow) GetHeaders(ctx context.Context, key string) (resilience.RateLimitHeaders, error) {
	sw.mu.Lock()
	defer sw.mu.Unlock()
...
func (sw *SlidingWindow) emitRateLimitEvent(key string, decision resilience.RateLimitDecision) {
	...
	resilience.EmitEvent(sw.eventEmitter, event)
}
