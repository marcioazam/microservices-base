...
func Find[T any](slice []T, predicate func(T) bool) option.Option[T] {
	for _, v := range slice {
		if predicate(v) {
...
func First[T any](slice []T) option.Option[T] {
	if len(slice) == 0 {
		return option.None[T]()
	}
	return option.Some(slice[0])
}

// Last returns the last element of the slice.
func Last[T any](slice []T) option.Option[T] {
	if len(slice) == 0 {
		return option.None[T]()
	}
	return option.Some(slice[len(slice)-1])
}
