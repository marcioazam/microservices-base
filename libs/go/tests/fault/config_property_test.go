...
func TestProperty_FunctionalOptions(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		cfg := fault.NewCircuitBreakerConfig(
			name,
			fault.WithFailureThreshold(threshold),
			fault.WithCircuitTimeout(time.Duration(timeoutSec)*time.Second),
		)
