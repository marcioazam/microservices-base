...
func (tb *TokenBucket) Allow(ctx context.Context, key string) (domain.RateLimitDecision, error) {
	tb.mu.Lock()
	defer tb.mu.Unlock()
...
func (tb *TokenBucket) GetHeaders(ctx context.Context, key string) (domain.RateLimitHeaders, error) {
	tb.mu.Lock()
	defer tb.mu.Unlock()
